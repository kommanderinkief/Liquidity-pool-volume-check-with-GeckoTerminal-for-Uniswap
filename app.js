...
async function getPools(pageNO) {
  const response = await axios.get(
    `https://app.geckoterminal.com/api/p1/arbitrum/pools?include=dex,dex.network,dex.network.network_metric,tokens&page=${pageNO}&include_network_metrics=true`
  );
...
function checkPool(pool) {
 // console.log(pool)
  ...
  const last_24h_cal = liquidity * 5;
...
if(liquidity < 800000 && liquidity > 100 && tradingVolume > 80 && tradingVolume < 300000){
  ...
  if (tradingVolume5m > last_5_min_cal) {
    const message = `5 min: ${pool.attributes.address} volume of $${tradingVolume5m} liquidity of $${liquidity}. pair name $${pool.attributes.name}  Retio: ${tradingVolume / liquidity}!\n`;
    console.log(message);
  //  bot.sendMessage('YOUR_TELEGRAM_CHAT_ID', message);
  }
  ...
  if (tradingVolume24hm > last_24h_cal) {
    const message = `Last 1 day Pool: ${pool.attributes.address} has trading volume of $${tradingVolume24hm} and liquidity of $${last_24h_cal}  pair name $${pool.attributes.name}  Retio: ${tradingVolume24hm / liquidity}!\n`;
    console.log(message);
  //  bot.sendMessage('YOUR_TELEGRAM_CHAT_ID', message);
  }
}
